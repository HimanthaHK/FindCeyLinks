...
interface Job {
  ...
  company: string;
  location: string;
  field: string; // This corresponds to the location type (Remote, Onsite, Hybrid)
  description: string;
...
export default function JobsPage() {
      ...
      try {
        ...
        querySnapshot.forEach((doc) => {
          ...
          const jobData: Job = {
            ...
            field: data.field ?? "Onsite", // Default to Onsite if not specified
  ...
  // Function to determine badge color based on field type
  const getLocationBadgeColor = (fieldType?: string) => {
    if (!fieldType) return "bg-gray-200 text-gray-800"; // fallback style
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[...Array(6)].map((_, i) => (
              <div
                key={i}
                className="bg-white/80 backdrop-blur-sm rounded-xl shadow-md p-6 animate-pulse border border-gray-200/50"
              >
        ...
        {jobs.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {jobs.map((job) => (
              ...
              >
                {/* Location type badge - using the field property */}
                <div
                  className={`absolute top-4 right-4 px-3 py-1 rounded-full text-xs font-semibold ${getLocationBadgeColor(
                    job.field
                  )}`}
                >
                  {job.field || "Unknown"}
